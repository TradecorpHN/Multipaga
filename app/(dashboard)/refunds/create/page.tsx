...
import { env } from '/home/kali/multipaga/src/presentation/lib/env-config'
...
const PayoutCreateSchema = z.object({
  ...
  description: z.string().max(1000).optional(),
  statement_descriptor: z.string().max(22).optional(),
  payout_type: z.enum(['bank', 'card', 'wallet']).default('bank'),
  billing: z.object({
    address: z.object({
      line1: z.string().optional(),
      line2: z.string().optional(),
...
const payoutRouter = router({
  ...
  list: protectedProcedure.input(
    PaginationSchema.merge(DateRangeSchema).extend({
      ...
      payout_type: z.array(z.enum(['bank', 'card', 'wallet'])).optional(),
    })
  ).query(async ({ input, ctx }) => {
    try {
  ...
  cancel: protectedProcedure.input(z.object({
    ...
    try {
      const { payoutId, ...cancelData } = input
      const payout = await ctx.hyperswitchClient.post(
  ...
  export: protectedProcedure
    .input(
      PaginationSchema.merge(DateRangeSchema).extend({
        customer_id: z.string().optional(),
        payout_status: z.array(z.string()).optional(),
        payout_type: z.array(z.string()).optional(),
        format: z.enum(['csv', 'json']).default('csv'),
      })
    )
    .mutation(async ({ input, ctx }) => {
